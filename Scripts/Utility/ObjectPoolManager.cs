...
namespace Rito.ObjectPooling
{
    ...
    public class ObjectPoolManager : MonoBehaviour
    {
        ...
        ***********************************************************************/
        ...
        public static ObjectPoolManager I
        {
            get
            {
                if (_instance == null)
                {
                    ...
                    if (_instance == null) ContainerObject.TryGetComponent(out _instance);
                }
                return _instance;
            }
        }
        ...
        private static GameObject ContainerObject
        {
            get
            {
                ...
                {
                    _containerObject = new ($"[Singleton] {nameof(ObjectPoolManager)}");
                    if (_instance == null) ContainerObject.TryGetComponent(out _instance);
                }
        ...
        ***********************************************************************/
        ...
        [Space]
        [SerializeField]
        private List<PoolObjectData> _poolObjectDataList = new (4);
        #endregion
        /***********************************************************************
        *                               Private Fields
        ***********************************************************************/
        ...
        // 디스폰 스케줄링 동기화 큐
        private readonly ConcurrentQueue<CloneScheduleInfo> _despawnScheduleQueue = new ();
        ...
        ***********************************************************************/
        ...
        private void Init()
        {
            ...
            _dataDict   = new (len);
            _poolDict   = new (len);
            _cloneDict  = new (len * PoolObjectData.INITIAL_COUNT);
        ...
        private IEnumerator PoolCleanerRoutine(KeyType key)
        {
            ...
            //WaitForSeconds wfs = new (_poolCleaningInterval);
            WaitForSeconds wfs = YieldCache.WaitForSeconds(_poolCleaningInterval);
        ...
        ***********************************************************************/
        ...
        private void RegisterInternal(PoolObjectData data)
        {
            ...
            Stack<GameObject> pool = new (data.maxObjectCount);
            for (int i = 0; i < data.initialObjectCount; i++)
            {
                ...
                _cloneDict.Add(clone, new (clone, pool)); // Clone-Data 캐싱
            }
            ...
            TestModeOnly(() =>
            {
                ...
                // 컨테이너 게임오브젝트 생성
                _t_ContainerDict.Add(data.key, new ());
        ...
        ***********************************************************************/
        ...
        public GameObject Spawn(KeyType key)
        {
            ...
            else
            {
                ...
                _cloneDict.Add(go, new (go, pool)); // Clone-Data 캐싱
